...
def load_features(csv_path: str) -> pd.DataFrame:
    """Load CSV, kiểm tra columns, trả về DataFrame chỉ gồm feature columns."""
...
def train(
    ...
    # Scale features — IsolationForest performs better on standardized data
    ...
    with mlflow.start_run() as run:
        ...
        # --- Log scaler artifact (cần để serve.py scale input đúng) ---
        ...
        print(f"[pipeline] Anomaly rate : {anomaly_rate:.4f}  ({contamination*100:.1f}% contamination)")
        print(f"[pipeline] Features     : {FEATURES}")

    # Set alias 'production' trên version mới nhất
...
def main():
    ...
    parser.add_argument("--n-estimators", type=int, default=100,
                        help="Number of IsolationForest estimators (default: 100)")
