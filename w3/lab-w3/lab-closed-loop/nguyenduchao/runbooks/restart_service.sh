...
#
# Dry-run:  print intended action, exit 0 (no side effect)
# Real run: docker restart (or docker start if stopped), verify running state
#
...
# If container exists (running or stopped) use restart; otherwise try start
...
echo "[restart_service] Waiting 5s for $CONTAINER to stabilise..."
sleep 5

STATUS=$(docker inspect --format '{{.State.Status}}' "$CONTAINER" 2>/dev/null || echo "missing")
if [[ "$STATUS" == "running" ]]; then
  echo "[restart_service] SUCCESS: $CONTAINER is running."
  exit 0
else
